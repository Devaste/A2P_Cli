...
def convert_single_image(avif_file, png_file, silent, log=False):
    """
    ...
    try:
        with Image.open(avif_file) as img:
            if is_greyscale(img):
                ...
                msg = f"[GREYSCALE 4bit GUI] Converted: {avif_file.name} -> {png_file.name}"
                if not silent:
                    print(msg)
                if log:
                    logging.info(msg)
                ...
                if is_gs_plus_one:
                    ...
                    msg = f"[GREYSCALE+{color.upper()}] Converted: {avif_file.name} -> {png_file.name}"
                    if not silent:
                        print(msg)
                    if log:
                        logging.info(msg)
                    ...
                    img.save(png_file, 'PNG', optimize=True)
                    msg = f"[COLOR] Converted: {avif_file.name} -> {png_file.name}"
                    if not silent:
                        print(msg)
                    if log:
                        logging.info(msg)
        ...
        msg = f"Failed to convert {avif_file.name}: {e}"
        if not silent:
            print(msg)
        if log:
            logging.error(msg)
...
def convert_avif_to_png(input_dir, output_dir=None, replace=False, recursive=False, silent=False, log=False):
    """
    ...
    if not avif_files:
        msg = "No .avif files found in the input directory."
        if not silent:
            print(msg)
        if log:
            logging.warning(msg)
...
def main():
    ...
    parser.add_argument('--log', nargs='?', const='20', help='Enable logging to convert.log. Optionally set log level: 10=DEBUG, 20=INFO, 30=WARNING, 40=ERROR, 50=CRITICAL. Default is 20 (INFO).')
    args = parser.parse_args()
    log_level = int(args.log) if args.log is not None else None
